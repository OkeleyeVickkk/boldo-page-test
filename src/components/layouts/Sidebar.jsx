...
export default function Sidebar() {
	...
	return (
		<aside
			...
			} bg-primary fixed transition-all xl:hidden duration-300 ease-in-out overflow-hidden right-0 h-screen w-full lg:hidden z-[99] px-6 pt-[80px] pb-5 shadow-2xl`}>
			<ul className="flex flex-col h-[calc(100dvh-80px)] pb-5">
				{navlinks.map((link, index) => {
					return (
						<li key={index} className="last:border-0 border-b border-white/40">
							<a
								href={link.link}
								className="text-xl font-medium hover:text-sec transition-all ease-in-out duration-300 font-manrope text-white py-6 flex">
								{link.title}
							</a>
						</li>
					);
